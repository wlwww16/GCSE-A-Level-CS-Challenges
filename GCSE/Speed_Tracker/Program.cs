...
namespace Speed_Tracker
{
    public class Program
    {
        public static void Main()
        {
            ...
            Console.WriteLine($"The speed of the car is {speed:0.00}mph."); // This would take a result, such as 123.4567, and cut off all but the first 2 decimals, thus 123.45.
        }

        private static DateTime? TryFormatTime(string time)
        {
            var splitTime = time.Split(":");

            if (splitTime.Length != 3) return null; // This check is a catch all for no numbers, only one, only two or more than three (all incorrect).
            

            if (int.TryParse(splitTime[0], out var hours) == false || int.TryParse(splitTime[1], out var minutes) == false || int.TryParse(splitTime[2], out var seconds) == false)
            {
